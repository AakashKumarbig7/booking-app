...
const AccessLevel = () => {
  ...
  const handleSelectAllUpdate = (e: any) => {
    ...
    setEditData((prev: any) => {
      ...
      Object.keys(updatedPermissions).forEach((type) => {
        updatedPermissions[type] = updatedPermissions[type].map(
          (item: any) => ({
            ...item,
            status: isChecked,
          })
  ...
  const handleCreate = async (newData: any) => {
    ...
    if (newData.name) {
      ...
      const finalData = [
        ...updatedData,
        { ...newData, permissions: permissionLists },
      ];
      setData(finalData);
  ...
  const handleRowClick = (rowData: any) => {
    ...
    const isAllTrue = Object.values(rowData.permissions[0]).every(
      (items: any) =>
        Array.isArray(items) && items.every((item: any) => item.status === true)
    );
              ...
              <div className="flex flex-col gap-2">
                <div className="flex gap-4">
                  <div className="flex flex-col gap-1 py-2 w-1/2">
                    <label className="text-gray-900 text-sm font-medium">Name</label>
...
>
  ...
  <SelectContent>
    ...
    <SelectItem value="Receptionist">Receptionist</SelectItem>
              ...
              <div className="w-full min-h-[75vh] h-[75vh] overflow-y-auto pt-6 py-8">
                <div className="flex justify-between items-start px-6">
                  <div className="w-1/2">
                    {permissionLists.map((category, categoryIndex) => (
                      <div key={categoryIndex} className="">
                        <p className="font-semibold text-lg text-[#1F2A37] mb-2">
                          Dashboard
                        </p>
                        {category.dashboard.map(
                          (permission: any, permissionIndex: any) => (
                            <div
                              key={permission.value}
                              className="flex items-center mb-2 gap-2 pl-6"
                            >
                              <input
                                type="checkbox"
                                id={permission.value}
                  ...
                  <div className="w-1/2">
                    {permissionLists.map((category, categoryIndex) => (
                      <div key={categoryIndex} className="">
                        ...
                        {category.productInventoryManagement.map(
                          (permission: any, permissionIndex: any) => (
                            <div
                              key={permission.value}
                              className="flex items-center mb-2 gap-2 pl-6"
                  ...
                  <div className="w-1/2">
                    {permissionLists.map((category, categoryIndex) => (
                      <div key={categoryIndex} className="">
                        ...
                        {category.loyaltyMembership.map(
                          (permission: any, permissionIndex: any) => (
                            <div
                              key={permission.value}
                              className="flex items-center mb-2 gap-2 pl-6"
                  ...
                  <div className="w-1/2">
                    {permissionLists.map((category, categoryIndex) => (
                      <div key={categoryIndex} className="">
                        ...
                        {category.discountManagement.map(
                          (permission: any, permissionIndex: any) => (
                            <div
                              key={permission.value}
                              className="flex items-center mb-2 gap-2 pl-6"
                  ...
                  <div className="w-1/2">
                    {permissionLists.map((category, categoryIndex) => (
                      <div key={categoryIndex} className="">
                        ...
                        {category.bookingManagement.map(
                          (permission: any, permissionIndex: any) => (
                            <div
                              key={permission.value}
                              className="flex items-center mb-2 gap-2 pl-6"
      ...
      <div className="w-full border border-border_color rounded-[8px] bg-white text-sm my-2">
        ...
        >
                ...
                <div className="flex items-center">
                  <div className="flex items-center">
                    {rowData.permissions.every(
                      (permission: any) =>
                        permission.dashboard.every(
                          (item: any) => item.status
                        ) &&
                        permission.userManagement.every(
                          (item: any) => item.status
                        ) &&
                        permission.courtManagement.every(
                          (item: any) => item.status
                        ) &&
                        permission.productInventoryManagement.every(
                          (item: any) => item.status
                        ) &&
                        permission.loyaltyMembership.every(
                          (item: any) => item.status
                        ) &&
                        permission.discountManagement.every(
                          (item: any) => item.status
                        ) &&
                        permission.reports.every((item: any) => item.status) &&
                        permission.settings.every((item: any) => item.status) &&
                        permission.bookingManagement.every(
                          (item: any) => item.status
                        )
                    )
                      ? "Full access"
                      : `${rowData.permissions
                          .reduce(
                            (count: number, permission: any) =>
                              count +
                              permission.dashboard.filter(
                                (item: any) => item.status
                              ).length +
                              permission.userManagement.filter(
                                (item: any) => item.status
                              ).length +
                              permission.courtManagement.filter(
                                (item: any) => item.status
                              ).length +
                              permission.productInventoryManagement.filter(
                                (item: any) => item.status
                              ).length +
                              permission.loyaltyMembership.filter(
                                (item: any) => item.status
                              ).length +
                              permission.discountManagement.filter(
                                (item: any) => item.status
                              ).length +
                              permission.reports.filter(
                                (item: any) => item.status
                              ).length +
                              permission.settings.filter(
                                (item: any) => item.status
                              ).length +
                              permission.bookingManagement.filter(
                                (item: any) => item.status
                              ).length,
                            0
                          )
          ...
          <Column width={200} fixed="right">
            <HeaderCell
              className="uppercase text-center pl-2"
              style={{ backgroundColor: "#f2f2f2" }}
            >
              Action
            </HeaderCell>
                    ...
                    >
                      <SheetHeader className="relative h-full">
                        ...
                        <SheetDescription>
                          ...
                          <div className="w-full min-h-[75vh] h-[75vh] overflow-y-auto pt-2 py-8">
                            <div className="flex flex-wrap justify-between items-start px-6 pb-5">
                              {[
                                ...
                                <div key={type} className="w-1/2 py-4">
                                  ...
                                  {editData?.permissions[0][type]?.map(
                  ...
                  <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
                    ...
                    <DialogContent className="w-[350px] rounded-lg">
                      <DialogHeader>
                        ...
                        <DialogDescription>
                          Are you absolutely want to delete?
                        </DialogDescription>
                      </DialogHeader>
